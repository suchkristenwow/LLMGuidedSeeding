...
from robot_client.msg import ProjectedDetectionArray
...
from planner_msgs.srv import (
    pci_initialization,
    pci_global,
    planner_go_to_waypoint_with_pose,
    pci_search,
    pci_searchRequest, 
    pci_searchResponse
)
from planner_msgs.msg import WaypointPlanStatus
...
class Robot:
    def __init__(self):
        ...
        rospy.loginfo("[robot] Subscribed to Ready: %s", is_ready_topic)
        ...
        rospy.loginfo("[robot] Subscribed to Odom: %s", odom_topic)
        ...
        rospy.loginfo("[robot] Subscribed to Path: %s", path_topic)
        ...
        rospy.loginfo(
            "[robot] Subscribed to Local Graph Points: %s",
            local_graph_points_topic,
        )
        ...
        rospy.loginfo(
            "[robot] Subscribed to Global Graph Points: %s",
            global_graph_points_topic,
        )
        ...
        rospy.loginfo(
            "[robot] Subscribed to Projected Objects: %s",
            all_objects_topic,
        )
        ...
        rospy.loginfo(
            "[robot] Subscribed to Current Object Detections: %s",
            current_object_detections_topic,
        )
        ...
        #TO DO: need to publish new termination message
        ...
        rospy.loginfo(
            "[robot] Subscribed to waypoint plan status topic: %s",
        ...
        rospy.loginfo(
            "[robot] waypoint marker topic: %s", waypoint_marker_topic
        )
    ...
    def add_endpoints(self):
        """
        Adds endpoints to the Flask server.
        """

        self.app.add_url_rule(
            "/get_image", "get_image", self.send_image, methods=["GET"]
        )
        ...
        self.app.add_url_rule(
            "/get_frontier_points",
            "get_frontier_points",
            self.send_frontier_points,
            methods=["GET"],
        )
        self.app.add_url_rule(
            "/get_graph_points",
            "get_graph_points",
            self.send_graph_points,
            methods=["GET"],
        )
        ...
        self.app.add_url_rule(
            "/get_all_objects",
            "get_all_objects",
            self.send_all_object_detections,
            methods=["GET"],
        )

        self.app.add_url_rule(
            "/get_current_object_detections",
            "get_current_object_detections",
            self.send_current_object_detections,
            methods=["GET"],
        )

        self.app.add_url_rule(
            "/get_object_detection_server_status",
            "get_object_detection_server_status",
            self.send_object_detection_status,
            methods=["GET"],
        )
        ...
        self.app.add_url_rule(
            ...
            "set_waypoint_threshold",
            self.set_waypoint_threshold,
            methods=["POST"],
        )

        self.app.add_url_rule(
            "/get_last_movement_time",
            "get_last_movement_time",
            self.send_last_movement_time,
            methods=["GET"],
        )

        self.app.add_url_rule(
            "/get_reached_end_goal",
            "get_reached_end_goal",
            self.send_reached_end_goal,
            methods=["GET"],
        )
        ...
        self.app.add_url_rule(
            "/object_labels",
            "receive_object_labels",
            self.receive_object_labels,
            methods=["POST"],
        )
        ...
        self.app.add_url_rule(
            "/set_object_interrupt",
            "set_object_interrupt",
            self.set_object_interrupt,
            methods=["POST"],
        )
        
        self.app.add_url_rule(
            "/trigger_planner_graph_update",
            "trigger_planner_graph_update",
            self.trigger_planner_graph_update,
            methods=["POST"]
            )
    ...
    def image_callback(self, front, left, right):
        ...
        # rospy.loginfo("[robot_client] Storing Image")
    ...
    def aggregate_and_filter_points(self):
        ...
        if all_points == []:
            rospy.loginfo(
                "[robot_client] No points to aggregate and filter"
            )
            return

        clustered_points = self.format_points(all_points, seq_ids, timestamps)
        rospy.loginfo("[robot_client] Points being sent. Numer: %d", len(clustered_points))
    ...
    def aggregate_points(self):
        ...
        timestamps = []
        seq_ids = []
        rospy.loginfo("[robot_client] Aggregating points")

        # Process local points
        rospy.loginfo(
            f"[robot_client] Local graph point length : {len(self.current_local_graph_points)}"
        )
        if (
            self.current_local_graph_points != []
        ...
        rospy.loginfo(
            f"[robot_client] Global graph points length: {len(self.current_global_graph_points)}"
        )
    ...
    def filter_points(self, all_points, seq_ids, timestamps):
        ...
        # roslaunch robot_client robot.launch object_detection_server_url:=http://0.0.0.0:5005 host_url:=http://0.0.0.0:5000 eps_dbscan:=5.0 min_samples:=5 median_filter_rate:=2 distance_threshold:=0.5 waypoint_threshold:=0.35 enable_bagging:=True
    ...
    def receive_waypoint(self):
        try:
            ...
            rospy.loginfo(f"[robot_client] Received waypoint: {waypoint}")
    ...
    def odom_callback(self, msg):
        # rospy.loginfo("[robot_client] Received odom")
        with self.odom_lock:
    ...
    def path_callback(self, msg):
        ...
        Returns:
            None
        """
        # rospy.loginfo("[robot_client] Received path")
        with self.path_lock:
            self.current_path = {}
    ...
    def local_graph_points_callback(self, msg):
        ...
        with self.local_graph_points_lock:
            rospy.loginfo("[robot_client] Received local graph points: %d", len(msg.poses))
            self.current_local_graph_points = {}
            for index, pose_stamped in enumerate(msg.poses):
                # Extracting the timestamp from each PoseStamped in the path
                timestamp_in_seconds = msg.header.stamp.to_sec()
    ...
    def global_graph_points_callback(self, msg):
        ...
        with self.global_graph_points_lock:
            rospy.loginfo("[robot_client] Received global graph points: %d", len(msg.poses))
            self.current_global_graph_points = {}
            for index, pose_stamped in enumerate(msg.poses):
                # Extracting the timestamp from each PoseStamped in the path
                timestamp_in_seconds = msg.header.stamp.to_sec()
    ...
    def frontier_points_callback(self, msg):
        ...
        with self.frontier_points_lock:
            rospy.loginfo("[robot_client] Received frontier points: %d", len(msg.poses))
            self.current_frontier_points = {}
            for index, pose_stamped in enumerate(msg.poses):
                # Extracting the timestamp from each PoseStamped in the path
                timestamp_in_seconds = msg.header.stamp.to_sec()
    ...
    def all_objects_callback(self, msg):
        ...
        with self.all_objects_lock:
            rospy.logdebug("[robot_client] Received All Objects objects")
            self.all_objects = {}
    ...
    def waypoint_plan_status_callback(self, msg):
        ...
        Returns:
            None
        """
        rospy.loginfo("[robot_client] Received waypoint plan status")
    ...
    def check_waypoint_status(self):
        ...
        Returns:
            None
        """
        rospy.loginfo_throttle(1, "[robot_client] Checking waypoint status")
        ...
        rospy.loginfo_throttle(1,
            "[robot_client] current_position: %s", current_position
        )
        rospy.loginfo_throttle(1,
            "[robot_client] current_waypoint: %s", current_waypoint
        )
        rospy.loginfo_throttle(1,
            "[robot_client] current_planned_waypoint: %s",
            current_planned_waypoint,
        )
        rospy.loginfo_throttle(1,
            "[robot_client] current_end_of_path: %s", current_end_of_path
        )
        ...
        rospy.loginfo_throttle(1,
            "[robot_client] distance_to_waypoint: %s", distance_to_waypoint
        )
        ...
        if not waypoint_ready:
            rospy.loginfo_throttle(1, "[robot_client] Waypoint not ready")
            ...
            rospy.loginfo_throttle(1,
                "[robot_client] Planner was unable to find planning point close to the waypoint"
            )
            self.waypoint_status = {
                "Distance": float("inf"),
                "Arrived": False,
    ...
    def receive_object_labels(self):
        rospy.loginfo("[robot_client] Received Object labels")
        try:
            # Extract GLIP label data from the request
            object_labels = request.get_json()
            rospy.loginfo("[robot_client] Received Object labels %s", object_labels)
            if self.use_glip:
        ...
        with self.global_graph_points_lock:
            with self.local_graph_points_lock:
                ...
                if self.filtered_graph_points != []:
                    rospy.loginfo("[robot_client] Sending filtered graph points")
                    return jsonify(self.filtered_graph_points)
        rospy.loginfo("[robot_client] Filtered graph points not available")
        return "Graph points not available", 404

    def send_frontier_points(self):
        with self.frontier_points_lock:
            if self.current_frontier_points != []:
                rospy.loginfo("[robot_client] Sending frontier points")
                return jsonify(self.current_frontier_points)
            rospy.loginfo("[robot_client] Frontier points not available")
    ...
    def send_can_plan_status(self):
        if self.waypoint_plan_status is not None:
            if self.check_waypoint_ready():
                rospy.loginfo(
                    f"[robot_client] Sending can plan status: {self.waypoint_plan_status.success}"
                )
