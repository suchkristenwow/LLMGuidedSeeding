...
from cog_explore_robot_client.msg import ObjectDetection, ObjectDetectionArray
import copy
...
class ObjectDetectionClient:
        ...
        while not rospy.is_shutdown():
            ...
            try:
                if len(self.image_queue) > 0 and self.object_list is not None:
                    ...
                    self.detection_lock.release()  # Release lock after getting the image
                    self.detect_objects(image_data)
                else:
                    self.detection_lock.release()
            except Exception as e:
                self.detection_lock.release()  # Ensure the lock is released if an error occurs
    ...
    def detect_objects(self, image_data):
        if True: #not self.image_queue.empty() and self.object_list is not None:
            # Encode the image to JPEG format
            #image = copy.deepcopy(self.image_queue.get())
            success, encoded_image = cv2.imencode(".jpg", image_data['image'])
            if not success:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.process_detection_results(image_data, response.json())
                else:
                    rospy.loginfo(
    ...
    def process_detection_results(self, image_data, response_data):
        ...
        projection_points = response_data["projection_points"]
        ...
        rospy.logdebug("projection_points: {}".format(projection_points))
        ...
        for i in range(len(labels)):
            ...
            x_full = int((x_coords[i][0] + x_coords[i][1]) / 2)
            detection.confidence = scores[i]
            detection.x = int(projection_points[i][0])
            detection.y = int(projection_points[i][1])
            ...
            # Get frame
            detection.header.frame_id = self.cam_frame_ids[self.get_frame_id(x_full)]
        ...
        if self.publish_detections_image_bool:
            self.publish_detections_image(image_data['image'], detection_array)
    ...
    def publish_detections_image(self, image, detections):
        ...
        for detection in detections.detections:
            ...
            projection_x = detection.x
            projection_y = detection.y
            cv2.circle(image, (projection_x, projection_y), 5, (255, 0, 0), -1)
